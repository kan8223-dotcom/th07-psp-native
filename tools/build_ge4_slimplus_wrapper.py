#!/usr/bin/env python3
"""Derive the GO-ME1 GE4 wrapper from the frozen model-3 wrapper.

The accepted wrapper is retained byte-for-byte as the input contract.  GO-ME1
changes only its two-instruction model predicate:
...
Thus negative model-query errors and Phat model 0 still reject, while every
positive Slim+ model is admitted.  The branch target and rejection return code
are unchanged, as are all HW-size, requested-size, export and module bytes.
"""
...
def derive(base: bytes) -> bytes:
    ...
    if actual_hash != BASE_SHA256:
        raise ValueError(
            f"base SHA-256 {actual_hash} != frozen {BASE_SHA256}"
        )
    ...
    candidate[
        MODEL_GATE_OFFSET : MODEL_GATE_OFFSET + len(MODEL3_GATE)
    ] = SLIMPLUS_GATE
