...
constexpr unsigned int kFrameBytes = kBufferWidth * kScreenHeight * 4;
...
class PspGuGraphics final : public ZunGraphics
{
  public:
    ...
    bool Init()
    {
        ...
        mListOpen = true;
        sceGuDrawBuffer(GU_PSM_8888, reinterpret_cast<void *>(0), kBufferWidth);
        ...
        auto *display = reinterpret_cast<volatile unsigned int *>((0x40000000u | edram) +
                                                                   kFrameBytes);
        for (int i = 0; i < kBufferWidth * kScreenHeight; ++i)
        {
            display[i] = 0xff000000u;
        }
...
#endif
        th07_psp_boot_note(g_Supervisor.cfg.windowed ? "display 4:3 fit 362x272"
                                                     : "display full stretch 480x272");
    ...
    void BindTexture(GfxTextureHandle handle) override
    {
        ...
        mBoundTexture = handle.id < kMaxTextures ? handle.id : 0;
    }
                         ...
                         const void *data) override
    {
        ...
        const bool surfaceCandidate = data && fmt == PIXEL_RGBA &&
        ...
        if (data)
        {
            if (fmt == PIXEL_RGBA && type == PIXEL_UNSIGNED_BYTE &&
    ...
    void MarkTextTexture(GfxTextureHandle handle)
    {
        ...
        if (!texture.pixels || texture.psm == GU_PSM_8888 || texture.borrowedSurfaceCache)
        {
        ...
        char message[96];
        std::snprintf(message, sizeof(message), "text atlas %u RGBA8888 %ux%u",
                            ...
                            const void *data) override
    {
        ...
        writeTexture.pixels = writePixels;
        ...
        for (int y = dstTop; y < dstBottom; ++y)
        {
            ...
            for (int x = dstLeft; x < dstRight; ++x)
            {
                ...
                WriteTexturePixel(writeTexture, y * texture.storageWidth + x, pixel[0], pixel[1],
                                  pixel[2], pixel[3]);
            }
        }
...
#endif
        if (writePixels == texture.updatePixels)
        {
    ...
    void ReadPixels(i32 x, i32 y, i32 width, i32 height, void *pixels) override
    {
        ...
        SubmitAndRestart();
        const unsigned int edram = reinterpret_cast<unsigned int>(sceGeEdramGetAddr());
        const auto *frame = reinterpret_cast<const unsigned int *>(
                         ...
                         i32 vertexStride) override
    {
        ...
        const unsigned int indexBytes = static_cast<unsigned int>(indexCount) * sizeof(u16);
        if (!EnsureListSpace(vertexBytes + indexBytes))
        {
            mError = true;
            return;
        }
        ...
        if (indexedQuads)
        {
            indices = static_cast<u16 *>(sceGuGetMemory(indexBytes));
            if (!indices)
            {
                mError = true;
                return;
  ...
  private:
                                 ...
                                 unsigned long long vblankEndUs)
    {
        ...
        char message[320];
        std::snprintf(message, sizeof(message),
                      "PERF S%d ST%d %u.%uFPS CPU%u.%u GE%u.%u VB%u.%u MAX%u.%u "
                      "D%u VI%u V%u L%u.%u LK%u UP%lluK C%u/%u HF%uK "
                      "ME%u SC%u TO%u MW%u",
                      g_Supervisor.curState, g_GameManager.currentStage, fps10 / 10, fps10 % 10,
                      cpu10 / 10, cpu10 % 10, ge10 / 10, ge10 % 10, vb10 / 10, vb10 % 10,
                      max10 / 10, max10 % 10, draws, inputVertices, vertices,
                      lists10 / 10, lists10 % 10,
