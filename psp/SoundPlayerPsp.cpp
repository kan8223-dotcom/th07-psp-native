...
__attribute__((noinline)) bool MixSfxBlock(i16 *block, u32 frames)
{
    ...
    mixJob.mixDivisor = 1;
    // Input zero preserves the BGM/fade block already assembled by the SC.
    // It is the only mutable source and therefore the only input requiring a
    // per-job cache writeback before ME reads it.
    ...
    if (mixed)
    {
        ...
        // Only the output thread writes this diagnostic counter.
        ++gSfxMixedBlocks;
    }
    return mixed;
}
