...
> 32MB RAMへ収めるため敵・弾・アイテム・Effectの最大同時数を64MB版より減らしており、
> 短いプールが原版より早く再利用へ回るため、同時使用数が上限未満でも生成先と更新順が
> 原作や64MB版から分岐します。テスターからも、
...
> リプレイファイルの作成環境だけでは説明できないゲーム進行の互換性問題として調査しています。
...
- PSP-1000版は敵・弾・アイテム・Effectの最大同時数を64MB版より減らしています。この差は
  高負荷弾幕のゲーム進行とリプレイ同期へ影響するため、原作相当の上限を保ったまま32MB RAMへ
  収める方法を再検討しています。
