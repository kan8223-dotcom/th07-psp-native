...
PSP_UNIFIED_EBOOT_SHA256 := 8a9225ec02bf12500420856d39815af03351fe2fd58da2a93029c2abd4300a87
PSP_RELEASE_1000_ANCHOR := release-anchors/psp1000-bounded-hotfix-20260903/EBOOT.PBP
PSP_RELEASE_1000_SHA256 := 5daee9fe5def74fed879a17712ec2654e5f7b0ddd6190ea2a1f70e3edc9ad842
...
ifeq ($(PSP_SFX_DIV1_FAST),1)
SRCS += psp/sfx_div1_fast.c
endif
...
CXXFLAGS += -DTH07_PSP_SLIMPLUS_ME_GATE
...
# A1-SAME is a sparse, read-only observer for the four authoritative bulk
# clear paths plus the separate per-Bullet bomb-clear traversal.  It is kept
# out of release/PSP-1000 profiles and never changes gameplay authority.
ifeq ($(PSP_PERF_A1_SAME),1)
ifneq ($(PSP_1000),0)
$(error PSP_PERF_A1_SAME is PSP-2000+-only and is not valid for PSP-1000)
endif
ifneq ($(PSP_PERF_DIAG),1)
$(error PSP_PERF_A1_SAME requires PSP_PERF_DIAG=1)
endif
ifneq ($(PSP_PERF_PROFILE),PERF_ACCEPT)
$(error PSP_PERF_A1_SAME requires PSP_PERF_PROFILE=PERF_ACCEPT)
endif
CXXFLAGS += -DTH07_PSP_PERF_A1_SAME
else ifneq ($(PSP_PERF_A1_SAME),0)
$(error PSP_PERF_A1_SAME must be 0 or 1)
endif

# A5-MEASURE attributes the existing SC SFX mixer timer to the exact compact
# ACCEPT windows.  It is an observer-only PSP-2000+ profile: the extra record
# and P99 sample buffer must never leak into a release or performance claim.
ifeq ($(PSP_PERF_SFX_MIX),1)
ifneq ($(PSP_1000),0)
$(error PSP_PERF_SFX_MIX is PSP-2000+-only and is not valid for PSP-1000)
endif
ifneq ($(PSP_PERF_DIAG),1)
$(error PSP_PERF_SFX_MIX requires PSP_PERF_DIAG=1)
endif
ifneq ($(PSP_PERF_PROFILE),PERF_ACCEPT)
$(error PSP_PERF_SFX_MIX requires PSP_PERF_PROFILE=PERF_ACCEPT)
endif
ifneq ($(PSP_PERF_AB_COMPARE),1)
$(error PSP_PERF_SFX_MIX requires PSP_PERF_AB_COMPARE=1)
endif
ifneq ($(PSP_PERF_DENSE_SLICE),1)
$(error PSP_PERF_SFX_MIX requires PSP_PERF_DENSE_SLICE=1)
endif
CXXFLAGS += -DTH07_PSP_PERF_SFX_MIX
else ifneq ($(PSP_PERF_SFX_MIX),0)
$(error PSP_PERF_SFX_MIX must be 0 or 1)
endif

# A5-SCALAR physically separates the runtime unity-divisor compose loop from
# the generic divider loop.  Keep it observer-off and confined to the exact
# PSP-3000 ACCEPT/AUDIO4M contract used for the hardware A/B.
ifeq ($(PSP_SFX_DIV1_FAST),1)
ifneq ($(PSP_1000),0)
$(error PSP_SFX_DIV1_FAST is PSP-3000-only)
endif
ifneq ($(PSP_MECC_AUDIO_4M),1)
$(error PSP_SFX_DIV1_FAST requires PSP_MECC_AUDIO_4M=1)
endif
ifneq ($(PSP_PERF_DIAG),1)
$(error PSP_SFX_DIV1_FAST requires PSP_PERF_DIAG=1)
endif
ifneq ($(PSP_PERF_PROFILE),PERF_ACCEPT)
$(error PSP_SFX_DIV1_FAST requires PSP_PERF_PROFILE=PERF_ACCEPT)
endif
ifneq ($(PSP_PERF_AB_COMPARE),1)
$(error PSP_SFX_DIV1_FAST requires PSP_PERF_AB_COMPARE=1)
endif
ifneq ($(PSP_PERF_SFX_MIX),0)
$(error PSP_SFX_DIV1_FAST must be measured observer-off)
endif
CXXFLAGS += -DTH07_PSP_SFX_DIV1_FAST
CFLAGS += -DTH07_PSP_SFX_DIV1_FAST
else ifneq ($(PSP_SFX_DIV1_FAST),0)
...
# GO-ME2 measures the exact M0 scalar kernel on SC and ME with a Wide wall
# timer, then adjusts only the established adaptive Item admission policy.
ifeq ($(PSP_ME_CLOCK_CALIBRATION),1)
ifneq ($(PSP_SLIMPLUS_ME_GATE),1)
$(error PSP_ME_CLOCK_CALIBRATION requires the GO-ME1 Slim+ contract)
endif
ifneq ($(PSP_ME_RENDER_WORKER),1)
$(error PSP_ME_CLOCK_CALIBRATION requires PSP_ME_RENDER_WORKER=1)
endif
ifneq ($(PSP_ME_ADAPTIVE_AUX_RENDER),1)
$(error PSP_ME_CLOCK_CALIBRATION requires PSP_ME_ADAPTIVE_AUX_RENDER=1)
endif
ifneq ($(PSP_ME_ITEM_PREFIX_SPLIT),1)
$(error PSP_ME_CLOCK_CALIBRATION requires PSP_ME_ITEM_PREFIX_SPLIT=1)
endif
ifneq ($(PSP_ME_ITEM_MOTION_UPDATE),1)
$(error PSP_ME_CLOCK_CALIBRATION requires PSP_ME_ITEM_MOTION_UPDATE=1)
endif
ifneq ($(PSP_USAGE_METER),1)
$(error PSP_ME_CLOCK_CALIBRATION requires PSP_USAGE_METER=1)
endif
ifneq ($(PSP_1000),0)
$(error PSP_ME_CLOCK_CALIBRATION is PSP-2000+ only)
endif
CXXFLAGS += -DTH07_PSP_ME_CLOCK_CALIBRATION
CFLAGS += -DTH07_PSP_ME_CLOCK_CALIBRATION
else ifneq ($(PSP_ME_CLOCK_CALIBRATION),0)
$(error PSP_ME_CLOCK_CALIBRATION must be 0 or 1)
endif
...
$(GE4_PROVEN_PRX): FORCE_GE4_PROVEN_PRX $(GE4_PROVEN_PRX_SOURCE) $(GE4_SLIMPLUS_PRX_BUILDER)
	...
	@test "$$(sha256sum "$@" | awk '{print $$1}')" = "$(GE4_SLIMPLUS_PRX_SHA256)"
else
...
ge4-proven-clean:
	rm -f psp/ge4_game_bridge.o psp/ge4_game_bridge.d \
		...
		psp/sfx_div1_fast.o psp/sfx_div1_fast.d \
		$(GE4_PROVEN_PRX)
...
.PHONY: psp1000-build psp1000-font-heap-control-build \
	...
	psp3000-a6v4w-d2b-a1-same-observer-build \
...
PSP_RID30_AB_ME_PERF_A1_SAME ?= 0
PSP_RID30_AB_ME_PERF_SFX_MIX ?= 0
PSP_RID30_AB_ME_SFX_DIV1_FAST ?= 0
...
# A5-MEASURE keeps the A7 gameplay/build flags and adds only the bounded SFX
# mixer observer.  The archived A7 binary cannot currently be reproduced from
# this workspace source (its DATA.PSP is 196 bytes older), so this target is an
# attribution build, not an A7 performance successor or adoption candidate.
psp3000-a7-a5-sfx-measure-build:
	$(MAKE) PSP_1000=0 PSP_RID30_AB_ME_UV16=0 \
		PSP_RID30_AB_ME_XYZ16=0 PSP_RID30_AB_ME_C1_GE_EXPERIMENT=0 \
		PSP_RID30_AB_ME_TRUSTED_SEED_AUTHORITY=0 \
		...
		PSP_RID30_AB_ME_POSITION_SOA_READ=0 \
		PSP_RID30_AB_ME_PERF_A1_SAME=0 \
		PSP_RID30_AB_ME_PERF_SFX_MIX=1 \
		PSP_RID30_AB_ME_TITLE_WORKSPACE=1 \
		PSP_RID30_AB_ME_TITLE_TRANSIENT=0 \
		PSP_RID30_AB_ME_TITLE_FONT_HOLE_SWAP=1 \
		PSP_RID30_AB_ME_LOCAL_FONT_SUBSET=1 \
		PSP_RID30_AB_ME_FONT_TAIL_ARCHIVE=1 \
		PSP_RID30_AB_ME_BUILD_ID=0x260902a5u \
		PSP_RID30_AB_ME_TITLE='TH07 A6V4W ST6 TAIL FIX' \
		psp3000-rid30-ab-me-build

# A5-SCALAR uses a fresh observer-off current-source control because the
# archived A7 DATA.PSP is 196 bytes older and cannot be rebuilt byte-exactly.
# Both targets inherit every A7 contract flag, title and build ID; the only
# candidate switch is the physical unity-divisor loop split.
psp3000-a7-a5-scalar-control-build:
	$(MAKE) PSP_RID30_AB_ME_SFX_DIV1_FAST=0 \
		psp3000-a6v4w-stage6-font-tail-fix-build

psp3000-a7-a5-scalar-build:
	$(MAKE) PSP_RID30_AB_ME_SFX_DIV1_FAST=1 \
		psp3000-a6v4w-stage6-font-tail-fix-build
...
# GO-ME2 differs from GO-ME1 only by the boot-time SC/ME Wide-wall
# calibration and its derived adaptive admission thresholds.  The title,
# dormant observer, Slim+ gates, selftests and every gameplay feature remain
# the GO-ME1 contract; a new build id keeps telemetry attributable.
pspgo-me2-clock-calibration-build:
	$(MAKE) PSP_SLIMPLUS_ME_GATE=1 PSP_1000=0 PSP_SHIKIGAMI=1 \
		PSP_MECC_AUDIO_4M=1 \
		PSP_RID30_AB_ME_UV16=0 PSP_RID30_AB_ME_XYZ16=0 \
		...
		PSP_RID30_AB_ME_SEED_SOA=0 \
		PSP_RID30_AB_ME_POSITION_SOA_SHADOW=0 \
		PSP_RID30_AB_ME_CLOCK_CALIBRATION=1 \
		PSP_RID30_AB_ME_TITLE_WORKSPACE=1 \
		PSP_RID30_AB_ME_TITLE_TRANSIENT=0 \
		PSP_RID30_AB_ME_TITLE_FONT_HOLE_SWAP=1 \
		PSP_RID30_AB_ME_LOCAL_FONT_SUBSET=1 \
		PSP_RID30_AB_ME_FONT_TAIL_ARCHIVE=1 \
		PSP_RID30_AB_ME_BUILD_ID=0x260902b2u \
		PSP_RID30_AB_ME_TITLE='TH07 PSP v0.2.1-beta' \
		psp3000-rid30-ab-me-build
...
# Sparse attribution-only successor to the hardware-working D2B build.  It
# changes no clear/bomb authority and emits one extra RAM-log line only for a
# window in which an A1 event occurred.
psp3000-a6v4w-d2b-a1-same-observer-build:
	$(MAKE) PSP_1000=0 PSP_RID30_AB_ME_UV16=0 \
		PSP_RID30_AB_ME_XYZ16=0 PSP_RID30_AB_ME_C1_GE_EXPERIMENT=0 \
		PSP_RID30_AB_ME_TRUSTED_SEED_AUTHORITY=0 \
		...
		PSP_RID30_AB_ME_POSITION_SOA_SHADOW=1 \
		PSP_RID30_AB_ME_POSITION_SOA_READ=1 \
		PSP_RID30_AB_ME_PERF_A1_SAME=1 \
		PSP_RID30_AB_ME_TITLE_WORKSPACE=1 \
		PSP_RID30_AB_ME_TITLE_TRANSIENT=0 \
		PSP_RID30_AB_ME_TITLE_FONT_HOLE_SWAP=1 \
		PSP_RID30_AB_ME_LOCAL_FONT_SUBSET=1 \
		PSP_RID30_AB_ME_FONT_TAIL_ARCHIVE=1 \
		PSP_RID30_AB_ME_BUILD_ID=0x260902a1u \
		PSP_RID30_AB_ME_TITLE='TH07 D2B A1 SAME OBS' \
		psp3000-rid30-ab-me-build
